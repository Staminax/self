...
class BranchSegment {
    ...
    draw(hoverEffect = 0, currentAnimationTime = Infinity) {
        ...
        const baseBrightness = config.baseColor.r;
        ...
        if (drawProgress < 1) {
            effectiveHover = Math.max(effectiveHover, 0.5);
        }
...
window.addEventListener('click', (e) => {
    ...
    for (const branch of allBranches) {
        const dist = branch.distanceToPoint(clickX, clickY);
        if (dist < closestDist) {
            closestDist = dist;
    ...
    if (closestBranch && closestDist < config.hoverRadius) {
        if (closestBranch.color) return;



        function colorSubtree(branch) {
            branch.color = { r: 255, g: 0, b: 0, neon: true };
            branch.width = branch.width * 1.5;

            if (branch.children) {
                branch.children.forEach(child => colorSubtree(child));
            }
        }

        colorSubtree(closestBranch);

        preservedRoots.push(closestBranch);
    }
});
...
const uiElements = document.querySelectorAll('.social-links, .scroll-btn, .skills-list, .language-switcher');
